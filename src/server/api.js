...
export function startServer() {
    ...
    return new Promise((resolve) => {
        serverInstance = app.listen(PORT, () => {
            console.log(`[INFO] TEST server running at http://localhost:${PORT}`);
            resolve();
        });
    });
}
    ...
    return new Promise((resolve) => {
        if (serverInstance) {
            serverInstance.close(() => {
                console.log('[INFO] TEST server stopped.');
                resolve();
            });
        }
    });
...
app.get('/appointments', (req, res) => {
    ...
    const message = results.length === 0 ? '[❌ DEBUG] No appointments found.' : `${results.length} appointment(s) found.`;
    sendResponse(res, 'SUCCESS', results, message, 200);
});

app.get('/test-timeout', (req, res) => {
    console.log('[INFO] Received request on /test-timeout, waiting 10 seconds...');
    setTimeout(() => {
...
app.post('/appointments', (req, res) => {
    ...
    if (!date || !time || !duration || !title) {
        return sendResponse(res, 'ERROR', null, "[❌ DEBUG] Missing required fields.", 400);
    }
...
app.patch('/appointments/:id', (req, res) => {
    ...
    if (isNaN(id)) return sendResponse(res, 'ERROR', null, '[❌ DEBUG] Invalid ID format.', 400);

    const appointmentIndex = appointments.findIndex(app => app.id === id);
    if (appointmentIndex === -1) {
        return sendResponse(res, 'ERROR', null, `[✅ DEBUG]Appointment with ID ${id} not found.`, 404);
    }
    ...
    sendResponse(res, 'SUCCESS', updatedAppointment, `[✅ DEBUG] Appointment ${id} updated successfully.`, 200);
});
...
app.delete('/appointments/:id', (req, res) => {
    ...
    sendResponse(res, 'SUCCESS', deletedAppointment, `[✅ DEBUG] Appointment ${id} deleted successfully.`, 200);
});
